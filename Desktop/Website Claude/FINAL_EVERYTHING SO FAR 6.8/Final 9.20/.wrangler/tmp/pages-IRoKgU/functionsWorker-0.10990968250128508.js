...
var TOKENS_KV_KEY = "qbo_tokens";
var QboNotConnectedError = class extends Error {
  static {
    __name(this, "QboNotConnectedError");
  }
};
var QboReauthRequiredError = class extends Error {
  static {
    __name(this, "QboReauthRequiredError");
  }
};
function requireKv(env2) {
  if (!env2.QBO_KV) {
...
async function saveTokens(env2, tokens) {
  await requireKv(env2).put(TOKENS_KV_KEY, JSON.stringify(tokens));
}
__name(saveTokens, "saveTokens");
async function loadTokens(env2) {
  const raw = await requireKv(env2).get(TOKENS_KV_KEY);
  if (!raw) return null;
  try {
...
__name(loadTokens, "loadTokens");
async function clearTokens(env2) {
  await requireKv(env2).delete(TOKENS_KV_KEY);
}
__name(clearTokens, "clearTokens");
function basicAuthHeader(env2) {
  const clientId = env2.QBO_CLIENT_ID;
  const clientSecret = env2.QBO_CLIENT_SECRET;
...
async function exchangeCodeForTokens(env2, code, redirectUri) {
  const resp = await fetch(TOKEN_ENDPOINT, {
    method: "POST",
    headers: {
      Authorization: basicAuthHeader(env2),
...
async function refreshTokens(env2, refreshToken) {
  const resp = await fetch(TOKEN_ENDPOINT, {
    method: "POST",
    headers: {
      Authorization: basicAuthHeader(env2),
...
async function getValidTokens(env2) {
  const tokens = await loadTokens(env2);
  ...
  let refreshed;
  try {
    refreshed = await refreshTokens(env2, tokens.refresh_token);
  } catch (e) {
    await clearTokens(env2);
  ...
  const updated = {
    ...
    expires_at: now + (Number(refreshed.expires_in || 3600) - 60) * 1e3
  };
  await saveTokens(env2, updated);
  return updated;
}
__name(getValidTokens, "getValidTokens");
function apiBase(tokens) {
  return tokens.environment === "sandbox" ? "https://sandbox-quickbooks.api.intuit.com" : "https://quickbooks.api.intuit.com";
}
__name(apiBase, "apiBase");
async function qboQuery(tokens, query) {
  const url = apiBase(tokens) + "/v3/company/" + encodeURIComponent(tokens.realmId) + "/query?query=" + encodeURIComponent(query) + "&minorversion=65";
...
__name(qboQuery, "qboQuery");
function escapeQboString(s) {
  return String(s).replace(/\\/g, "\\\\").replace(/'/g, "\\'");
}
__name(escapeQboString, "escapeQboString");
...
async function onRequestGet(context2) {
  ...
  const backToAdmin = /* @__PURE__ */ __name((status, message) => {
  ...
  if (error3) return backToAdmin("error", error3);
  if (!code || !state || !realmId) return backToAdmin("error", "missing_params");
  if (!env2.QBO_KV) return backToAdmin("error", "not_configured");
  const stateKey = "oauth_state:" + state;
  const validState = await env2.QBO_KV.get(stateKey);
  if (!validState) return backToAdmin("error", "invalid_state");
  await env2.QBO_KV.delete(stateKey);
  let tokenData;
  try {
    tokenData = await exchangeCodeForTokens(env2, code, env2.QBO_REDIRECT_URI);
  } catch (e) {
    return backToAdmin("error", "token_exchange_failed");
  }
  const now = Date.now();
  await saveTokens(env2, {
    access_token: tokenData.access_token,
    refresh_token: tokenData.refresh_token,
    expires_at: now + (Number(tokenData.expires_in || 3600) - 60) * 1e3,
    realmId,
    environment: env2.QBO_ENVIRONMENT === "sandbox" ? "sandbox" : "production",
    connected_at: now
  });
...
// api/qbo/connect.js
var AUTH_ENDPOINT = "https://appcenter.intuit.com/connect/oauth2";
async function onRequestGet2(context2) {
  const { env: env2 } = context2;
  if (!env2.QBO_CLIENT_ID || !env2.QBO_REDIRECT_URI) {
    return new Response(
      "QuickBooks is not configured yet. Set QBO_CLIENT_ID, QBO_CLIENT_SECRET, and QBO_REDIRECT_URI in this Cloudflare Pages project's environment variables, and bind a KV namespace named QBO_KV.",
  ...
  await env2.QBO_KV.put("oauth_state:" + state, "1", { expirationTtl: 600 });
  const authUrl = new URL(AUTH_ENDPOINT);
  authUrl.searchParams.set("client_id", env2.QBO_CLIENT_ID);
  authUrl.searchParams.set("response_type", "code");
  authUrl.searchParams.set("scope", "com.intuit.quickbooks.accounting");
  authUrl.searchParams.set("redirect_uri", env2.QBO_REDIRECT_URI);
...
async function onRequestGet3(context2) {
  ...
  let tokens;
  try {
    tokens = await getValidTokens(env2);
  } catch (e) {
  ...
  const addEstimates = /* @__PURE__ */ __name((queryResponse) => {
...
async function onRequestPost2(context2) {
  ...
  try {
    ...
    return jsonResponse2({ error: "invalid_json" }, 400);
  }
  const subject = (body && body.subject || "").toString().trim();
  const text = (body && body.text || "").toString();
  const html = body && body.html ? body.html.toString() : void 0;
  if (!subject || !text) {
    return jsonResponse2({ error: "missing_fields", detail: "subject and text are required" }, 400);
  }
  if (!env2.RESEND_API_KEY) {
    return jsonResponse2({ error: "not_configured", detail: "RESEND_API_KEY is not set on this Pages project" }, 500);
  }
  const toAddress = env2.SUMMARY_TO_EMAIL || "victor@mirmont.info";
  const fromAddress = env2.SUMMARY_FROM_EMAIL || "Mirmont Admin <onboarding@resend.dev>";
  let resp, data;
  try {
    ...
    return jsonResponse2({ error: "network_error", detail: e.message }, 502);
  }
  if (!resp.ok) {
    return jsonResponse2({ error: "send_failed", detail: data }, 502);
  }
  return jsonResponse2({ ok: true, id: data.id, to: toAddress });
}
__name(onRequestPost2, "onRequestPost");

// ../.wrangler/tmp/pages-IRoKgU/functionsRoutes-0.7485873160712798.mjs
var routes = [
  {
